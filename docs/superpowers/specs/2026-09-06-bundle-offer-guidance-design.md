...
When several hunt-fit listings sit with one seller, the useful move is a multi-item checkout and a **bundle offer** on the goods total (for example five Mama / Mamalicious pieces listed at ~227 GBP, offer submitted at 187 GBP, fees still at checkout). The hunt already finds keep-bundles, value hauls, and near hauls and shows listing sum plus checkout extra — but it never suggests **what goods total to type into Vinted**. The buyer still has to invent the bid by hand.
...
3. As a buyer, I want gym carts aimed at about 30 GBP delivered per useful item, so that ordinary gym wardrobe hauls stay cheap after fees.
4. As a buyer, I want maternity carts aimed at about 50 GBP delivered per useful item, so that Mama-style hauls match real accepted offers.
...
12. As a buyer, I want ntfy alerts for value hauls and keep-bundles to include “offer ~X GBP”, so that I can act from my phone.
...
17. As a buyer, I want config knobs for target GBP/item and haircut bounds, so that I can tune without code changes.
...
1. **Single formula seam.** One pure function (bundle-offer guidance) takes listing sum, checkout extra, useful-item count, category target (gym vs maternity), and kind class (alerted vs near). It returns suggested offer (whole GBP), whether the offer is weak, and the target delivered-per-item used. No I/O inside the function.

2. **Back-solve then clamp.** Compute the maximum goods total that still meets `(offer + checkout_extra) / n_useful ≤ target_per_item`. Round down to whole GBP. Then clamp into `[listing_sum * (1 - max_haircut), listing_sum * (1 - min_haircut)]`. If listing sum is zero or useful count is below 2, return no suggestion (omit fields).

3. **Targets.** Gym target delivered per useful item: 30 GBP. Maternity: 50 GBP. Live in config next to existing value-haul caps (or a dedicated `bundle_offer` config block that the pure function reads via passed numbers — prefer passing resolved numbers into the pure function so tests do not load config).
...
- A second cart optimizer that subsets items to hit the GBP/item target
...
- Example primary source for maternity feel: seller cart ~227.15 GBP listing / submitted 187 GBP goods (~18% off), buy-now delivered picture ~310 GBP — fees are context; the typed offer is goods-only.
