...
async function buildSnapshot() {
  const { loadIndexedFromDb, indexBundleOpportunities } = require("./scoredDb");

  const [deals, bundlesRaw, pool, run, seen, indexedFile, dbIndexed] = await Promise.all([
  ...
  return {
    ...
    sellers: sellerRows,
    watches: [...new Set(finds.map((f) => f.watch).filter(Boolean))].sort(),
    run: {
