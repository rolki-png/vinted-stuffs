/**
 * Desk workflow dispatch starts the hunt.
...
async function testHuntDispatchSendsWorkflowInputs() {
	...
	assert.equal("legacy_active_v2" in payload.inputs, false);
	assert.equal(result.full_sweep, true);
}

await testHuntDispatchSendsWorkflowInputs();
