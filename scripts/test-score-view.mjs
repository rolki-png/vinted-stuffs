...
const leftover = { deal_score: 9, value_band: "hunt" };
const malformedV2 = {
	score_version: 2,
	buy_score: 101,
...
assert.equal(scoreLabel({ deal_score: 9 }), "—");
assert.equal(scoreLabel(malformedV2), "Invalid score");
assert.equal(scoreLabel({}), "—");
assert.equal(numericScore(v2), 87);
assert.equal(numericScore(leftover), 0);
assert.equal(numericScore(malformedV2), 0);
assert.equal(filterScore(v2), 87);
assert.equal(filterScore(leftover), null);
assert.equal(filterScore(malformedV2), null);
assert.equal(scoreScaleLabel(v2), "Score /100");
assert.equal(scoreScaleLabel(leftover), "Unscored");
assert.equal(scoreScaleLabel(malformedV2), "Invalid score");
...
assert.equal(matchesScoreFilter(v2, "85"), true);
assert.equal(matchesScoreFilter(v2, "v2:85"), true);
assert.equal(matchesScoreFilter({ ...v2, buy_score: 84 }, "85"), false);
assert.equal(matchesScoreFilter(leftover, "85"), false);
assert.equal(matchesScoreFilter(leftover, "legacy"), false);
assert.equal(matchesScoreFilter(malformedV2, ""), true);
assert.equal(matchesBandFilter(v2, "keep"), true);
assert.equal(matchesBandFilter(v2, "v2:keep"), true);
assert.equal(matchesBandFilter(leftover, "keep"), false);
assert.equal(matchesBandFilter(leftover, "legacy:hunt"), false);
assert.deepEqual(
	keepCounts([
...
assert.deepEqual(
	sortFinds(
		[
			{ id: "leftover", ...leftover, deal_score: 10 },
			{
				id: "ranked-low-score",
				...v2,
				buy_score: 80,
	...
	["unranked-high-score", "ranked-low-score", "leftover", "malformed"],
);
assert.deepEqual(
	sortFinds(
		[
	...
	["rank-one", "rank-two", "leftover", "malformed"],
);
assert.deepEqual(
	sortFinds(
		[
			{ id: "leftover-high", ...leftover, deal_score: 10 },
			{ id: "v2-high", ...v2, buy_score: 95 },
			{ id: "v2-low", ...v2, buy_score: 60 },
			{ id: "leftover-low", ...leftover, deal_score: 1 },
			{ id: "malformed", ...malformedV2 },
		],
		"score-asc",
	).map((row) => row.id),
	["v2-low", "v2-high", "leftover-high", "leftover-low", "malformed"],
);
assert.deepEqual(
	sortFinds(
		[
...
assert.deepEqual(
	[
		...
		{ id: "cheap", ...leftover, price_num: 10 },
	]
...
assert.deepEqual(vetoScoreContext(leftover), {});
for (const row of [
...
assert.deepEqual(vetoPayload(8, "bought", leftover, "poor_value"), {
	item_id: 8,
	status: "bought",
	hunt_name: null,
	brand: null,
...
assert.deepEqual(vetoPayload(9, null, leftover), {
	item_id: 9,
	clear: true,
});
