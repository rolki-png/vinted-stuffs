...
import { loadVetoMap, applyToFinds } from "./listingVetoes.ts"
import {
  matchesBandFilter,
  matchesScoreFilter,
  sortFinds,
...
function bandSql(band, params) {
  if (!band) return "TRUE"
  const [scale, value] = band.split(":", 2)
  ...
  if (scale === "legacy") {
    params.push(value)
    return `(best.score_version IS DISTINCT FROM 2) AND best.value_band = $${params.length}`
  }
  return "TRUE"
}
...
function minScoreSql(minScore, params) {
  ...
  if (minScore === "legacy") {
    return `(best.score_version IS DISTINCT FROM 2) AND best.deal_score IS NOT NULL`
  }
  const match = /^v2:(\d+)$/.exec(minScore)
...
function sortSql(sort) {
  ...
  const tier = `CASE WHEN joined.score_version = 2 AND joined.buy_score IS NOT NULL AND joined.buy_score BETWEEN 0 AND 100 THEN 2
    WHEN joined.score_version IS DISTINCT FROM 2 THEN 1 ELSE 0 END`
  const score = `CASE WHEN joined.score_version = 2 AND joined.buy_score IS NOT NULL THEN joined.buy_score
    ELSE COALESCE(joined.deal_score, -1) END`
  switch (sort) {
    case "score-asc":
      return `${vetoOrd} ASC, ${tier} ASC, ${score} ASC, joined.item_id ASC`
    case "price-asc":
    ...
    case "score-desc":
    default:
      return `${vetoOrd} ASC, ${tier} DESC, ${score} DESC, joined.rank_position ASC NULLS LAST, joined.item_id ASC`
  }
}
...
async function buildFindsPage(rawFilters = {}) {
  ...
  return {
    ...paged,
    source: dbIndexed?.source || (indexed.length ? "indexed_scores.json" : "json"),
  }
}
    ...
    const summary = await withClient(async (client) => {
      ...
      const sql = `
        ...
        SELECT
          ...
          COUNT(*) FILTER (
            WHERE score_version IS DISTINCT FROM 2
              AND COALESCE(deal_score, 0) >= 9
              AND value_band IN ('steal', 'hunt')
          )::int AS keeps_legacy
        FROM joined
      `
      ...
      return {
        ...
        keeps_v2: row.keeps_v2 ?? 0,
        keeps_legacy: row.keeps_legacy ?? 0,
