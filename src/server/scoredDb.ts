...
function indexBundleOpportunities(exportRows, { minItems = 2 } = {}) {
	const bySeller = new Map();
	for (const row of exportRows) {
		if (row.reason === "unavailable during backfill") continue;
		if (!isV2(row)) continue;
		if (row.hunt_fit !== true) continue;
		if (!["bundle", "good", "keep", "exceptional"].includes(row.buy_band))
			continue;
	...
	for (const [sid, rows] of bySeller) {
		...
		for (const r of rows) {
			...
			if (
				!prev ||
				Number(displayScore(r) || 0) > Number(displayScore(prev) || 0)
			) {
				best.set(id, r);
			}
		}
		...
		const country =
			members.find((r) => r.seller_country)?.seller_country || null;
		...
		Object.assign(
			row,
			offerFields(listingSum, extra, members.length, { kind, watchName }),
		);
		out.push(applyToRow(row));
	}
	out.sort((a, b) =>
		String(b.kept_at || "").localeCompare(String(a.kept_at || "")),
	);
	return out;
}
...
export { databaseUrl, exportRow, indexBundleOpportunities, loadIndexedFromDb };
